...
class Main(Chare):
    ...
    def __init__(self, args):
        ...
        charm.thisProxy.updateGlobals({'mainProxy': self.thisProxy}, '__main__', ret=True).get()  # store the main proxy

        print('Running wave2d on', charm.numPes(), 'processors')

        # Create new array of worker chares
        self.arrayProxy = Array(Wave, (chareArrayWidth, chareArrayHeight))
        # Start the computation
        self.arrayProxy.begin_iteration(False)
    ...
    def iterationCompleted(self, im, pos, imSize):
        ...
        if self.count == chareArrayWidth * chareArrayHeight:
            if self.iteration == self.total_iterations:
                ...
                self.arrayProxy.begin_iteration(restart)
...
class Wave(Chare):
    ...
    def requestNextFrame(self):
        ...
        mainProxy.iterationCompleted(data, (sx,sy), (w,h))
