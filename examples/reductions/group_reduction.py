...
class Main(Mainchare):
  def __init__(self, args):
    ...
    print "Running reduction example on", CkNumPes(), "processors"
...
class TestGroup(Group):
  ...
  def doReduction(self):
    print "TestGroup element on PE", CkMyPe(), " is starting its contributions."
    ...
    num = [10.1, 13.4]
    self.contribute(num+[float(self.thisIndex)], charm.ReducerType.sum_float, Main.done_float, ro.mainProxy)
    ...
    self.contribute([4.2, 13.1], charm.ReducerType.sum_float, TestArray.reduceGroupToArray, ro.arrayProxy[(0,)])
    # test contributing to TestArray (broadcast)
    self.contribute(-4, charm.ReducerType.sum_int, TestArray.reduceGroupToArrayBcast, ro.arrayProxy)
    # test contributing to TestGroup[0]
    self.contribute([5, 7, -3, 0], charm.ReducerType.sum_int, TestGroup.reduceGroupToGroup, self.thisProxy[0])
    # test contributing to TestGroup (broadcast)
    self.contribute(-4.2, charm.ReducerType.sum_double, TestGroup.reduceGroupToGroupBcast, self.thisProxy)
  ...
  def reduceGroupToGroupBcast(self, reduction_result):
    ...
    self.contribute(None, charm.ReducerType.nop, Main.done_group_to_group_bcast, ro.mainProxy)


class TestArray(Array):
  def __init__(self):
    super(TestArray,self).__init__()
    print "TestArray", self.thisIndex, "created on PE", CkMyPe()
  ...
  def reduceGroupToArrayBcast(self, reduction_result):
    ...
    self.contribute(None, charm.ReducerType.nop, Main.done_group_to_array_bcast, ro.mainProxy)
